...
Read-Host 'Please press ENTER' | Out-Null
.\LogParser.exe -i:W3C "SELECT cs-uri-stem AS Url, WIN32_ERROR_DESCRIPTION(sc-win32-status) AS Error, Count(*) AS Total FROM $filePath WHERE (sc-win32-status > 0) GROUP BY Url, Error ORDER BY Total DESC" -o:CSV
...
Read-Host 'Please press ENTER' | Out-Null
.\LogParser.exe -i:W3C "SELECT cs-uri-stem AS Url, Count(*) AS Hits, DIV(MUL(1.0, SUM(time-taken)), Hits ) as AvgTime, SQRROOT ( SUB ( DIV ( MUL(1.0, SUM(SQR(time-taken)) ), Hits ) , SQR(AvgTime) ) ) AS StDev FROM $filePath GROUP BY URL ORDER BY AvgTime" -o:CSV
Write-Host "All unique URLs:"
Read-Host 'Please press ENTER' | Out-Null
.\LogParser.exe -i:W3C "SELECT DISTINCT TO_LOWERCASE(cs-uri-stem) AS Url, Count(*) AS Hits FROM $filePath WHERE sc-status=200 GROUP BY Url ORDER BY Url" -o:CSV
